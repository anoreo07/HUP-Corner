...
export default async function HoaHocPage({ searchParams }: { searchParams?: { page?: string } }) {
  const page = searchParams?.page ? Number(searchParams.page) : 1;
  const perPage = 12;

  const { data, totalPages } = await getApprovedDocumentsPaginated('HOA_HOC', page, perPage);
