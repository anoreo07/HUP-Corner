...
export default async function DuocHocPage({ searchParams }: { searchParams?: { page?: string } }) {
  const page = searchParams?.page ? Number(searchParams.page) : 1;
  const perPage = 12;

  const { data, totalPages } = await getApprovedDocumentsPaginated('DUOC_HOC', page, perPage);
