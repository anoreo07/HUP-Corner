...
export default async function CongNgheSinhHocPage({ searchParams }: { searchParams?: { page?: string } }) {
  const page = searchParams?.page ? Number(searchParams.page) : 1;
  const perPage = 12;

  const { data, totalPages } = await getApprovedDocumentsPaginated('CONG_NGHE_SINH_HOC', page, perPage);
