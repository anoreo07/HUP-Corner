...
/**
 * Proxy API to download a single chunk from Telegram
 * This avoids server memory issues by piping the stream directly
 */
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: error.message || 'Chunk download failed' },
      { status: 500 }
    );
  }
}
